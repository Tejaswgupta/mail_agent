...
def _local_dir() -> Path:
    today = date.today()
    d = settings.DOWNLOADS_DIR / f"{today.year}" / f"{today.month:02d}" / f"{today.day:02d}"
...
def process_download(download: "Download", email_id: str, true_filename: str | None = None) -> dict | None:
    """Save a Playwright Download, hash it, parse it, store metadata. Returns meta dict or None."""
    local_dir = _local_dir()
    
    # Use true_filename from DOM if available, else fallback to Playwright's suggestion
    filename = true_filename if true_filename else download.suggested_filename
    
    # Sometimes Zoho downloads have no extension in the suggested filename
    if not filename or (len(filename) > 30 and '-' in filename and '.' not in filename):
...
def cleanup_downloads(older_than_days: int = 7) -> None:
    ...
    today = date.today()
    for year_dir in settings.DOWNLOADS_DIR.iterdir():
