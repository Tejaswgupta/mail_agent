...
def test_cleanup_downloads_removes_old(tmp_path, monkeypatch):
    ...
    old_dir = tmp_path / "Votum" / str(old.year) / f"{old.month:02d}" / f"{old.day:02d}"
...
def test_cleanup_downloads_keeps_recent(tmp_path, monkeypatch):
    ...
    today = date.today()
    recent = tmp_path / "Votum" / str(today.year) / f"{today.month:02d}" / f"{today.day:02d}"
